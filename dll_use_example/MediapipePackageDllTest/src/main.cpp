...
void showImage(int rows, int cols, int type, void* data, int channel) {

	std::cout << "rows：" << rows << ",cols:" << cols << ",type :" << type << ",channel:" << channel << std::endl;

	cv::Mat origin_image(cv::Size(cols, rows), type, (uchar*)data);

	//深拷贝
	cv::Mat copyMat;
	origin_image.copyTo(copyMat);

	uchar* pImageData = origin_image.data;
	...
	int* pdetect_result = new int[5];
	
		if (mediapipeHolisticTrackingDll.m_MediapipeHolisticTrackingDetectFrame(copyMat.cols, copyMat.rows,type, pImageData, pdetect_result, pImageData))
		{
		...
		std::cout << "pdetect_result[4]：" << pdetect_result[4] << std::endl;
	if (1 == pdetect_result[4]) {


		cv::Mat result_image(cv::Size(cols, rows), type, (uchar*)pImageData);

		imshow("test_opencv_srtup", result_image);
		out = 1;
...
void HolisticTrackingDllTest()
{
	...
	while (true)
	{
		...
		showImage(copyMat.rows, copyMat.cols, copyMat.type(), pImageData, copyMat.channels());
		copyMat = NULL;
...
void HolisticTrackingTest() {

	std::string str = "C:\\Users\\fs-dev1\\Pictures\\Redocn_2013073116200888.jpg";

	//std::string str = "C:\\Users\\fs-dev1\\Pictures\\manGlassFace.png";

	cv::Mat image = cv::imread(str);
	showImage(image.rows, image.cols, image.type(), image.data, image.step);
	while (0 == out) {
		std::cout << "out："  <<out <<std::endl;
		showImage(image.rows, image.cols, image.type(), image.data, image.step);
...
int main()
{
	...
	//InitHolisticTrackingDll();
	HolisticTrackingDllTest();

	//std::thread th1(HolisticTrackingTest);
	//th1.join();
