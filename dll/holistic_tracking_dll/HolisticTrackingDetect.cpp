...
int GoogleMediapipeDetect::HolisticTrackingDetect::DetectImage(int image_width, int image_height, int type, void* image_data, int* detect_result, void * detect_result_data)
{
	if (!m_bIsInit)
		return 0;

	absl::Status run_status = Mediapipe_RunGraph_Direct(image_width, image_height,type, image_data, detect_result,detect_result_data);
	if (!run_status.ok()) {
		return 0;
	}
	return 1;
...
absl::Status GoogleMediapipeDetect::HolisticTrackingDetect::Mediapipe_RunGraph_Direct(
	int image_width,
	int image_height,
	int type,
	void* image_data,
	int* detect_result,
	void* detect_result_data
)
{
	// 等待图处于空闲状态
	//m_Graph.WaitUntilIdle();
	...
	/*----- 1 构造cv::Mat对象 -----*/
	cv::Mat camera_frame(cv::Size(image_width, image_height), type, (uchar*)image_data);
	...
	if (m_pVideoPoller != nullptr)
	{
		...
		if (m_pVideoPoller->QueueSize() != 0)
		{
			...
			if (!m_pVideoPoller->Next(&videoPacket))
			{
				...
				if (NULL == output_frame_mat.data) {
					std::cout << "output_frame_mat.data == null" << std::endl;
				}

				//detect_result_data = output_frame_mat.data;
				//std::cout << "outout_frame_mat size:" << sizeof((uchar*)return_mat.data)/sizeof(uchar) << std::endl;

				//std::cout << "rows:" << output_frame_mat.rows <<",cols:" << output_frame_mat.cols<< std::endl;
				memmove((uchar*)detect_result_data, (uchar*)output_frame_mat.data, sizeof(uchar) * output_frame_mat.total()*output_frame_mat.channels());
				detect_result[4] = 1;
